...
const addedIndices = (added, modifiedArray, original) => {
  modifiedArray.forEach(item => {
    const exists = original.find(i => _.isEqual(i, item));
...
const changedIndices = (changed, modifiedArray, original) => {
  original.forEach((item, index) => {
    const unchanged = modifiedArray.find(i => _.isEqual(i, item));
    const sameLength = _.isEqual(original.length, modifiedArray.length);
...
const removedIndices = (removed, workingArray, modifiedArray, original) => {
  original.forEach(item => {
    const inNewArray = modifiedArray.findIndex(i => _.isEqual(item, i));
...
const mergeArray = (nextVal, original, newVal) => {
  ...
  if (!_.isEmpty(changed)) {
    const withChanges = workingArray.map(item => {
      const updated = changed.find(changed => _.isEqual(changed.oldVal, item));
...
export const changedKeys = (changed, modifiedObj, original) => {
  Object.keys(original).forEach(key => {
    ...
    const newVal = modifiedObj[key];
    const exists = newVal;
    const hasChange = !_.isEqual(newVal, originalVal);
...
export const mergeChanges = (changed, key, workingState, prevVal, workingVal) => {
  ...
  const isArr = _.isArray(newVal);
  const isObj = _.isObject(newVal);
